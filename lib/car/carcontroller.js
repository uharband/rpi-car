...
let right_forward_pin = 'P1-32';	// IN1 - Forward Drive
let right_backward_pin = 'P1-33';	// IN2 - Reverse Drive
// Motor B, Left Side GPIO CONSTANTS
let left_forward_pin = 'P1-12';	// IN1 - Forward Drive
let left_backward_pin = 'P1-35';	// IN2 - Reverse Drive
...
let requested_operation = 'none';
let last_executed = 'none';
let executing = false;
let dryMode = false;
let currentSpeed = 0;
...
let requested_direction = direction.none;
...
function setRequestedState(requestedSpeed, requstedDirection) {
    requested_speed = requestedSpeed;
    switch(requstedDirection.toLowerCase()){
        case 'up':
            requested_direction = direction.forward;
            break;
        case 'down':
            requested_direction = direction.backwards;
            break;
        case 'right':
...
function applyState() {
    logger.info('applyState - entered. currentSpeed=' + currentSpeed + ', requested_speed=' + requested_speed);
    if(current_direction === requested_direction && requested_speed === currentSpeed){
        return;
    }

    if(requested_speed !== currentSpeed){
...
function increaseForwardSpeed(){
    logger.info('entered increaseForwardSpeed. currentSpeed=' + currentSpeed + ', speedMax=' + config.speedMax + ', v_state=' + v_state.toString());
...
function increaseBackwardSpeed(){
    logger.info('entered increaseBackwardSpeed. currentSpeed=' + currentSpeed + ', speedMax=' + config.speedMax + ', v_state=' + v_state.toString());
...
function backwards() {
    ...
    current_direction = direction.backwards;
...
function execute(operation) {
    ...
    if(operation === 'increaseForwardSpeed' || operation === 'increaseBackwardSpeed' || operation === 'applyState'){
        last_executed = '';
    }
...
module.exports.setRequestedState = setRequestedState;
